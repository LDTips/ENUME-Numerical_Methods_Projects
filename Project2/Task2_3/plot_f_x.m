function plot_f_x(step)
    ...
    for i = 0 : size(f_x_arr, 2) % Iterate from 0 until the f_x_arr is filled
        f_x_arr(i+1) = eval_f_x(boundaries(1) + i * step); % start from f(boundaries(1))
        % Then next value will be f(2 + current_iteration * step)
    end
    x_range = boundaries(1):step:boundaries(2); % Define the x_axis values.
    % difference between every element being 'step'
    mark = (abs(f_x_arr) < 1e-02); %  Define the condition for 0 point marking
